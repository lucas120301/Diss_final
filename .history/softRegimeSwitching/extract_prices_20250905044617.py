...
def is_last_px_label(s) -> bool:
    ...
    return (
        ...
        or s in {"px_last", "last_px", "lastpx", "last price", "px last"}
    )
...
def to_datetime_series_mixed(series: pd.Series) -> pd.Series:
    ...
    # IMPORTANT: convert to plain float64 NumPy array to avoid pandas metadata issues
    nums_arr = nums.astype("float64").to_numpy()
    dt_from_nums_idx = pd.to_datetime(nums_arr, errors="coerce", unit="D", origin="1899-12-30")
    # Wrap back into a Series aligned to original index
    dt_from_nums = pd.Series(dt_from_nums_idx, index=series.index)
    ...
    # No infer_datetime_format (deprecated behavior); pandas now parses strictly by default
