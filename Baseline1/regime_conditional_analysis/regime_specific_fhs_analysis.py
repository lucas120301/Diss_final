#!/usr/bin/env python3
"""
Regime-Specific FHS Analysis

Run FHS separately for each regime period to compare how Baseline1 vs 
Regime-Switching models perform within their respective regimes.

This provides regime-specific risk profiles and period-by-period comparison.
"""
...
def filtered_historical_simulation_regime_specific(historical_returns, vol_forecasts, 
                                                  regime_mask, confidence_level=0.05, 
                                                  window_size=250):
    """
    Regime-Specific FHS: Only use historical returns from the SAME regime
    
    This creates regime-specific historical distributions for VaR estimation
    """
    var_estimates = []
    ...
    for i in range(len(vol_forecasts)):
        ...
        # Only use historical returns from the same regime
        regime_window = regime_mask[i-window_size:i]
        hist_returns_window = historical_returns[i-window_size:i]
        
        # Filter to only same-regime historical returns
        same_regime_returns = hist_returns_window[regime_window == regime_mask[i]]
        
        # Need minimum observations for stable VaR
        if len(same_regime_returns) < 50:
            # Fall back to all historical returns if insufficient regime-specific data
            same_regime_returns = hist_returns_window
        
        hist_vol = np.std(same_regime_returns, ddof=1)
        
        if hist_vol <= 1e-10:
            hist_vol = 1e-10
        
        # Standardize without mean subtraction (exactly like baseline)
        standardized_returns = same_regime_returns / hist_vol
        ...
        # VaR estimate from regime-specific distribution
...
def analyze_regime_specific_fhs(index_name):
    """
    Run regime-specific FHS analysis:
    - Extract each regime's time periods
    - Run FHS using only that regime's historical data
    - Compare Baseline1 vs Regime models within each regime period
    """
    print(f"\n{'='*60}")
    print(f"REGIME-SPECIFIC FHS ANALYSIS: {index_name}")
    print(f"{'='*60}")
    ...
    for window_size in [5, 11, 22]:
        ...
        for regime_id in range(4):  # Skip regime 4
            ...
            # Extract ALL periods where this regime is active
            regime_mask = regimes_aligned == regime_id
            regime_periods = np.where(regime_mask)[0]
            
            if len(regime_periods) < 500:  # Need sufficient data for regime-specific FHS
                print(f"    Insufficient regime periods: {len(regime_periods)}")
                continue
            
            print(f"    Found {len(regime_periods)} periods in Regime {regime_id}")
            
            # Create regime-specific datasets
            regime_returns = returns_aligned[regime_mask]
            
            # For predictions, we need to map the regime test periods to the prediction indices
            # This is complex - let's use the regime-specific test predictions directly
            regime_split = int(0.7 * len(regime_periods))
            test_regime_periods = regime_periods[regime_split:]
            test_returns_regime = returns_aligned[test_regime_periods]
            
            # Align predictions with test periods
            ...
            # Get corresponding baseline predictions (approximate mapping)
            ...
            print(f"    Test period: {len(test_returns_aligned)} observations")
            
            # Run regime-specific FHS for multiple confidence levels
            ...
            for conf_level, conf_name, expected_rate in zip(confidence_levels, confidence_names, expected_rates):
                print(f"    --- {conf_name} VaR (Regime-Specific Distribution) ---")
                
                # Create regime mask for the full aligned period
                full_regime_mask = regimes_aligned
                
                # Run regime-specific FHS on Regime predictions
                if len(regime_preds_aligned) > 250:
                    var_regime = filtered_historical_simulation_regime_specific(
                        returns_aligned, regime_preds_aligned, full_regime_mask,
                    ...
                    if len(var_regime) > 0:
                        ...
                        results.append({
                            ...
                            'Model_Type': 'REGIME_SWITCHING_REGIME_SPECIFIC_FHS',
                            'Analysis_Type': 'Regime-Specific Historical Distribution',
                        ...
                        print(f"      Regime Model (Regime-Specific FHS): {violation_rate_regime*100:.2f}% ({np.sum(violations_regime)}/{len(violations_regime)})")
                
                # Run regime-specific FHS on Baseline1 predictions
                if len(baseline_preds_regime) > 250:
                    var_baseline = filtered_historical_simulation_regime_specific(
                        returns_aligned, baseline_preds_regime, full_regime_mask,
                        confidence_level=conf_level, window_size=250
                    )
                    ...
                    if len(var_baseline) > 0:
                        ...
                        results.append({
                            ...
                            'Window': window_size,
                            'Confidence_Level': conf_name,
                            'Model_Type': 'BASELINE1_REGIME_SPECIFIC_FHS',
                            'Analysis_Type': 'Regime-Specific Historical Distribution',
                        ...
                        print(f"      Baseline1 Model (Regime-Specific FHS): {violation_rate_baseline*100:.2f}% ({np.sum(violations_baseline)}/{len(violations_baseline)})")
    
    return results

def main():
    """Run regime-specific FHS analysis for all indices"""
    
    print("="*80)
    print("REGIME-SPECIFIC FHS ANALYSIS")
    print("="*80)
    print("\nThis analysis runs FHS separately for each regime using")
    print("regime-specific historical distributions to compute VaR.")
    print("\nKey innovation: Historical simulation uses only returns")
    print("from the SAME regime to build the risk distribution.")
    print("\nThis captures regime-specific risk characteristics.")
    ...
    for index_name in ["SPX", "NDX", "RTY"]:
        try:
            results = analyze_regime_specific_fhs(index_name)
    ...
    if all_results:
        ...
        output_file = "regime_specific_fhs_analysis_results.csv"
        results_df.to_csv(output_file, index=False)
        
        print(f"\n{'='*80}")
        print("REGIME-SPECIFIC FHS RESULTS")
        print(f"{'='*80}")
        ...
        print(f"\n{'='*60}")
        print("REGIME-SPECIFIC FHS MODEL PERFORMANCE")
        print(f"{'='*60}")
        ...
        print(f"\n{'='*60}")
        print("REGIME-SPECIFIC FHS: BASELINE1 vs REGIME-SWITCHING")
        print(f"{'='*60}")
        ...
        for conf_level in ['90%', '95%', '99%']:
            ...
            for index_name in ["SPX", "NDX", "RTY"]:
                for regime_id in range(4):
                    ...
                    baseline_results = regime_results[regime_results['Model_Type'] == 'BASELINE1_REGIME_SPECIFIC_FHS']
                    regime_model_results = regime_results[regime_results['Model_Type'] == 'REGIME_SWITCHING_REGIME_SPECIFIC_FHS']
                    ...
                    if len(baseline_results) > 0 and len(regime_model_results) > 0:
                        ...
                        print(f"  {index_name} Regime {regime_id}: Baseline1={baseline_avg_viol:.2f}% vs Regime-Specific={regime_avg_viol:.2f}%")
